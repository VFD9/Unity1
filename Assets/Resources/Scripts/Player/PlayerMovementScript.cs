...
public class PlayerMovementScript : MonoBehaviour 
{
	...
	void Awake()
	{
		...
		_walkSound.volume = 0.5f;
		_runSound.volume = 0.5f;
	}
	...
	*/
	...
	void PlayerMovementLogic()
	{
		...
		if (grounded) 
			...
			rb.AddRelativeForce (Input.GetAxis ("Horizontal") * accelerationSpeed  * 0.5f * Time.deltaTime, 0, Input.GetAxis ("Vertical") * accelerationSpeed * 0.5f * Time.deltaTime);
	...
	*/
	...
	void Jumping()
	{
		if (Input.GetKey(KeyCode.Space) && grounded)
		{
			...
			if (_jumpSound)
				PlayerManager.Instance.pms._jumpSound.Play();
		}

		if (!grounded)
        {
			PlayerManager.Instance.pms._walkSound.mute = true;
			PlayerManager.Instance.pms._runSound.mute = true;
		}
		else if (grounded)
        {
			PlayerManager.Instance.pms._walkSound.mute = false;
			PlayerManager.Instance.pms._runSound.mute = false;
		}
	}
	...
	*/
	...
	void WalkingSound()
	{
		...
		{	
			if (currentSpeed > 1)
			{
				if (maxSpeed >= 3 && maxSpeed < 5)
				{
					if (!_walkSound.isPlaying)
					{
						PlayerManager.Instance.pms._walkSound.Play();
						PlayerManager.Instance.pms._runSound.Stop();
					}
				}
				else if (maxSpeed >= 5)
				{
					if (!_runSound.isPlaying && !_jumpSound.isPlaying)
					{
						PlayerManager.Instance.pms._walkSound.Stop();
						PlayerManager.Instance.pms._runSound.Play();
					}
				}
			}
			else
	...
	private void OnTriggerEnter(Collider other)
	{
		...
		{
			PlayerManager.Instance.AddBuff(other.gameObject);
			other.gameObject.SetActive(false);

			if (PlayerManager.Instance.Buff.Count <= 1 && PlayerManager.Instance.onBuff == true)
				maxSpeed += 5;

			StartCoroutine(SpeedFix(other.gameObject));
		}
	
		if (other.gameObject.name == "Ammo")
		{
			if (PlayerManager.Instance.gunscript.amountOfBulletsPerLoad ==
			PlayerManager.Instance.gunscript.bulletsInTheGun)
			{
				PlayerManager.Instance.gunscript.bulletsIHave += 40;
			}
			else if (PlayerManager.Instance.gunscript.amountOfBulletsPerLoad >
				PlayerManager.Instance.gunscript.bulletsInTheGun + 20)
			{
				PlayerManager.Instance.gunscript.bulletsInTheGun += 20;
			}
			else if (PlayerManager.Instance.gunscript.bulletsInTheGun + 20 >
				PlayerManager.Instance.gunscript.amountOfBulletsPerLoad)
			{
				float overAmmo = PlayerManager.Instance.gunscript.bulletsInTheGun + 20;
				float remainAmmo = overAmmo - PlayerManager.Instance.gunscript.amountOfBulletsPerLoad;
				PlayerManager.Instance.gunscript.bulletsInTheGun = PlayerManager.Instance.gunscript.amountOfBulletsPerLoad;
				PlayerManager.Instance.gunscript.bulletsIHave += remainAmmo;
			}
	...
	IEnumerator SpeedFix(GameObject _Obj)
    {
		while(true)
        {
			yield return null;

			if (PlayerManager.Instance.BuffTime <= 0.0f)
			{
				Destroy(_Obj);

				if (PlayerManager.Instance.gunscript.running == true)
                {
					maxSpeed = 5;
					break;
                }
	...
	*/
	...
	void Crouching()
	{
		...
		{
			if (Input.GetKey(KeyCode.C) && Input.GetKeyDown(KeyCode.LeftShift) == false)
			{
	...
	*/
	...
	{		
		if (currentWeapo == "gun") 
		{
			...
			if (_hitSound)
				PlayerManager.Instance.pms._hitSound.Play();
			else
