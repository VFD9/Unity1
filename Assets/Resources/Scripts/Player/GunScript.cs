...
public class GunScript : MonoBehaviour 
{
	...
	void Awake()
	{
		mls = PlayerManager.Instance.mls;
		...
		pmS = PlayerManager.Instance.pms;
		...
		playerHP = PlayerManager.Instance.playerstat;
	}
	...
	*/
	void Update()
	{
		if (PlayerManager.Instance.Player.transform.localScale.y <= 0.6f)
		{
			Animator anim = GameObject.Find("FPS_Character_prefab").GetComponent<Animator>();
	...
	*/
	...
	void PositionGun()
	{
		//if (transform.position == Vector3.negativeInfinity)
		//{
	...
	private void ShootMethod()
	{
		if(waitTillNextFire <= 0 && !reloading && pmS.maxSpeed < 5)
		{
			if(bulletsInTheGun > 0)
			{
				...
				{
					PlayerManager.Instance.gunscript.shoot_sound_source.Play();
					PlayerManager.Instance.gunscript.shoot_sound_source.volume = 0.5f;
				}
				else
	...
	*/
	...
	IEnumerator Reload_Animation()
	{
		if(bulletsIHave > 0 && bulletsInTheGun < amountOfBulletsPerLoad && !reloading/* && !aiming*/){
			...
			if (/*reloadSound_source.isPlaying == false && */reloadSound_source != null)
            {
				...
				{
					PlayerManager.Instance.gunscript.reloadSound_source.Play();
					PlayerManager.Instance.gunscript.reloadSound_source.volume = 0.6f;
				}
				else
			...
			if (meeleAttack == false && pmS.maxSpeed != runningSpeed) 
			{
				...
				if (player.GetComponent<PlayerMovementScript>()._freakingZombiesSound)
				{
					...
					player.GetComponent<PlayerMovementScript>()._freakingZombiesSound.volume = 0.5f;
				}
				else
	...
	void OnGUI()
	{
		if(!currentBullet)
		{
			...
			{
				print("Couldn't find the Currentbullet ->" + ex.StackTrace.ToString());
			}
		}

		if (mls && currentBullet)
			currentBullet.text = bulletsInTheGun.ToString(); 

		if (!totalBullet)
		{
			try
			{
				totalBullet = GameObject.Find("TotalBullet").GetComponent<Text>();
			}
			catch (System.Exception)
			{
				//print("Couldn't find the TotalBullet ->" + ex.StackTrace.ToString());
			}
		}
		if (mls && totalBullet)
			totalBullet.text = bulletsIHave.ToString(); 
