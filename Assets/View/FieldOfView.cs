...
public class FieldOfView : MonoBehaviour
{
	...
	IEnumerator CheckTargetat()
	{
		while (true)
		{
			...
			Collider[] ColList = Physics.OverlapSphere(transform.position, Radius, TargetMask); // 움직이는 객체 확인용으로 적합
			...
			foreach (Collider element in ColList)
			{
				...
				if (Vector3.Angle(transform.forward, Direction) < (ViewAngle * 0.5f))
				{
					...
					if (!Physics.Raycast(transform.position, Direction, fDistance, ObstacleMask)) // ObstacleMask가 아닐때 TargetList에 추가
	...
	public CastInfo GetCastInfo(float _Angle)
	{
		...
		if (Physics.Raycast(transform.position, Direction, out hit, Radius, ObstacleMask)) // 고정된 객체 확인용으로 적합
		{
			// ** 맞았는지 안맞았는지 확인
			Info.Hit = true;
		...
		else
		{
			...
			Info.Point = Direction * Radius + transform.position;
	...
	void GetVertex()
	{
		...
		//_MeshFilter.transform.rotation = Quaternion.Euler(0.0f, 0.0f, 0.0f);

		//Debug.Log(fAngle + ", " + OffsetAngle);
